...
import { getSectionAndPageType } from '../../shared/lib/url';
...
export function filterRows(rows: LogRow[], filters: Partial<FiltersState>): LogRow[] {
  ...
  return rows.filter((row) => {
    ...
    if (safeFilters.sections.length && !safeFilters.sections.includes(row.section)) return false;
...
export function buildFilterOptions(rows: LogRow[]) {
  return {
    ...
    sections: Array.from(new Set(rows.map((row) => row.section))).sort((a, b) => a.localeCompare(b, 'ru')),
...
export function parseAllSitemapFiles(files: TextFilePayload[]): SitemapUrl[] {
  return files.flatMap((file) => parseSitemapXml(file.content).map((url) => ({ ...url, group: file.name })));
}

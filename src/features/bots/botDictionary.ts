...
export function getAgentGroupForBotName(botName: string, knownGroups?: AgentGroup[]): AgentGroup | undefined {
  if (knownGroups?.length) return knownGroups[0];

  const normalized = botName.toLowerCase();
  for (const group of agentGroups) {
    if (botSignatures[group].some((signature) => normalized.includes(signature.toLowerCase()))) return group;
  }
  return undefined;
}

export function getBotColor(botName: string, knownGroups?: AgentGroup[]): string {
  const group = getAgentGroupForBotName(botName, knownGroups);
  return group ? agentGroupChartColors[group] : chartColors[0];
}
